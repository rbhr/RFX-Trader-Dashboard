...
docker exec rfx-mysql mysqldump -u rfx -p"${MYSQL_PASSWORD}" rfx_trader \
...
# Caddy data (SSL certs)
docker run --rm \
  -v rfx-trader-dashboard_caddy_data:/data:ro \
...
# Caddy config
docker run --rm \
  -v rfx-trader-dashboard_caddy_config:/data:ro \
  -v "$VOLUMES_DIR":/backup \
