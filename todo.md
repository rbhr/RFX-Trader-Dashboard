...
## 🧪 Test Infrastructure — Isolated Test Database
**Status:** 🔴 Not started — picking up week of 2026-06-08. Background: the Vitest suite writes/deletes real rows against `process.env.DATABASE_URL`, which in deployment is the live `rfx_trader` DB. A guard (`server/test-db-guard.ts`, v2.0.5) now hard-fails the suite unless pointed at a test DB, so `pnpm test` is currently blocked until this is done.
